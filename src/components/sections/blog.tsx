'use client';
import { FilterType } from '@/components/features/filter';
...
export const Blog = ({ posts }: BlogProps) => {
  ...
  const filteredPosts = useMemo(() => {
    return posts.filter((post) => {
      const matchesSearch = [post.title, post.summary].some((text) =>
        text.toLowerCase().includes(searchTerm.toLowerCase())
      );
      const matchesType = filterType === 'all' || post.label === filterType;
  ...
  const { totalPosts, totalPages, currentPosts } = useMemo(() => {
    ...
    return { totalPosts, totalPages, currentPosts };
  ...
  return (
    <section className="pt-14 md:pt-16 lg:pt-24">
      ...
      <div className="min-h-[15rem] md:min-h-[30rem] flex flex-col">
            ...
            {currentPosts.map((post) => (
              ...
              >
                ...
                <CardContent className="pt-6 flex flex-col flex-1">
                  ...
                  <p className="mt-2 line-clamp-3 text-sm text-gray-600">{post.summary}</p>
                </CardContent>
                <CardFooter className="border-t border-gray-100 px-6 py-4">
                  <Link
                    ...
                    aria-label={`Read ${post.title} article`}
                    title={`Read ${post.title} article`}
                    className="flex items-center text-sm font-medium text-navy transition-colors duration-300 hover:text-navy focus:outline-none"
                  >
